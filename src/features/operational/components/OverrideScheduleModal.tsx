...
export default function OverrideScheduleModal({
  ...
  const handleTimeChange = (field: "customOpenTime" | "customCloseTime", value: string) => {
  ...
  return (
    <div>
      ...
      >
        ...
        Tambah Schedule Override
      </button>

      {/* DRAWER / FORM BOX */}
      {isOpen && (
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 px-6 text-left shadow-md space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              <label htmlFor="overrideDate" className="block text-xs font-bold text-slate-700">
              ...
              <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-3">
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-rose-700">1. Full Closed</span>
                    <input
                      type="radio"
                      name="status"
                      value="CLOSED"
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-amber-800">2. Custom Hours</span>
                    <input
                      type="radio"
                      name="status"
                      value="MODIFIED"
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-emerald-800">3. Force Open</span>
                    <input
                      type="radio"
                      name="status"
                      value="OPEN"
            ...
            {status === "MODIFIED" && (
              <div className="flex flex-wrap items-center gap-3 rounded-xl border border-amber-200 bg-amber-50/40 p-3">
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold text-amber-900">Jam Buka:</span>
                  <input
                    type="text"
                    maxLength={5}
                    placeholder="08:00"
                    ...
                    onChange={(e) => handleTimeChange("customOpenTime", e.target.value)}
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold text-amber-900">Jam Tutup:</span>
                  <input
                    type="text"
                    maxLength={5}
                    placeholder="13:00"
                    ...
                    onChange={(e) => handleTimeChange("customCloseTime", e.target.value)}
                    className="w-16 rounded-lg border border-amber-300 bg-white px-2.5 py-1 text-center text-xs font-bold text-slate-900 outline-none"
                  />
                </div>
            ...
            {(status === "CLOSED" || status === "MODIFIED") && (
              <div className="space-y-1">
                <label htmlFor="reasonNotice" className="block text-xs font-bold text-slate-700">
                  Catatan / Teks Pengumuman Running Marquee:
                  {status === "CLOSED" && <span className="text-rose-500">* (Wajib)</span>}
                </label>
                <input
                  type="text"
                  id="reasonNotice"
